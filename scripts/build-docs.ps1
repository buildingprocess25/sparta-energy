...
$docxPath = Join-Path (Get-Location) "Dokumen_Logika_dan_Hirarki_Kalkulator_Lampu.docx"
$rtfPath  = Join-Path (Get-Location) "Dokumen_Logika_dan_Hirarki_Kalkulator_Lampu.rtf"
$guid = [System.Guid]::NewGuid().ToString("N").Substring(0, 8)
$tempDir = Join-Path (Get-Location) "temp_build_$guid"
...
$flowText = @"
TAHAP 1: Hitung Target Daya Listrik (4.0 - 5.0 W/m2)
...
TAHAP 3: Penentuan Baris Sampling (Proteksi W/m2 Minimum)
...
[void]$bodySb.Append( (Make-Bullet "2. Prioritas Total Daya Listrik (Anti-Redup)" "Sistem memprioritaskan toko selalu cukup terang (0% kasus redup). Jika ada dimensi nanggung antara sedikit kurang terang (< 4.0 W/m2) vs sedikit lebih terang (> 5.0 W/m2), sistem selalu memilih membulatkan ke atas (lebih terang).") )
[void]$bodySb.Append( (Make-Bullet "3. Prioritas Vertikal (Panjang Toko / Anti-Belang)" "Jarak antar baris dibatasi maksimal 1.9 m. Jika jarak baris melebihi 1.9 m, sistem wajib menambah 1 baris terlepas dari rasio W/m2 akan naik sedikit.") )
...
$headers1 = @("jsMin in [0.3, 0.6]", "jsMax in [0.3, 0.6]", "jsMin-1 in [0.3, 0.6]", "Keputusan LPB", "Alasan & Dampak Prioritas")
$rows1 = @(
...
[void]$bodySb.Append( (Make-Bullet "1. Prinsip 3 Anti Terbukti Efektif" "Kalkulator berhasil mengeliminasi 0% kasus toko redup (< 4.0 W/m2) dan 0% kasus lampu mepet rak (< 0.3 m).") )
...
[void]$bodySb.Append( (Make-Bullet "3. Wacana Batas Bawah (1.6 m)" "Jika di masa depan batas bawah 1.6 m diterapkan dengan hirarki deviasi (W/m2 > JS > JB), maka 84% hasil hitungan dan visual denah tidak akan berubah sama sekali.") )
...
[System.IO.File]::WriteAllText((Join-Path $tempDir "_rels\.rels"), $rootRelsXml, [System.Text.Encoding]::UTF8)
[System.IO.File]::WriteAllText((Join-Path $tempDir "word\_rels\document.xml.rels"), $docRelsXml, [System.Text.Encoding]::UTF8)
...
$targetDocx = Join-Path (Get-Location) "Dokumen_Logika_dan_Hirarki_Kalkulator_Lampu_v2.docx"
...
# Try copying over to the original filename if user closed Word
try {
    Copy-Item $targetDocx $docxPath -Force -ErrorAction SilentlyContinue
} catch { }

Write-Host "Berhasil membuat file DOCX tervalidasi di: $targetDocx"
