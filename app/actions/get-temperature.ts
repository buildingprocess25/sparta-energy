...
export async function getTemperature(lat: string, lng: string) {
  try {
    ...
    const suhuMaksimalHarian: (number | null)[] = data.daily?.temperature_2m_max || [];
    const suhuTersaring = suhuMaksimalHarian.filter((suhu): suhu is number => suhu !== null);
    ...
    return { error: "Gagal mengambil data suhu lokasi. Pastikan koordinat valid dan koneksi stabil." };
  }
}
