...
const filterParamKeys = ["role", "branch"] as const
...
export function AdminUserFilters({ branches, roles }: AdminUserFiltersProps) {
  ...
  const roleOptions = useMemo(
    () => roles.map((item) => ({ value: item.toLowerCase(), label: item })),
    [roles]
  )
    ...
    <div className="flex flex-col gap-2 xl:flex-row xl:items-center">
      ...
      <div className="flex min-w-0 flex-1 items-center gap-2">
        <Filters
          ...
          allowMultiple={false}
          idPrefix="admin-user-filters"
          size="default"
          className="min-w-0 flex-1"
          trigger={
            <Button
              id="admin-user-filter-trigger"
              type="button"
              variant="outline"
            >
              <IconFilter aria-hidden="true" data-icon="inline-start" />
              Filter
            </Button>
          }
          i18n={{
